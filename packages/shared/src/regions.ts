 ...
 * 시·도와 그 안의 시·군·구.
 *
 * **왜 손으로 적어 두는가.** 행정구역은 몇 년에 한 번 바뀌는 정도라, API 를
 * 붙여 매번 받아 올 만큼 자주 변하지 않는다. 대신 바뀌면 여기를 고쳐야
 * 한다는 뜻이므로, 최근 바뀐 것은 아래에 적어 둔다.
 *
 * 최근 변경
 *  - 2023-06  강원도 → 강원특별자치도
 *  - 2023-07  경상북도 군위군 → 대구광역시 편입
 *  - 2024-01  전라북도 → 전북특별자치도
 *
 * **이름은 정식 표기로 적는다**(`성남시`). 공고는 `성남` 처럼 줄여 적는 일이
 * 많은데, 맞춰 보는 쪽에서 끝의 시·군·구를 떼고 견주므로 여기서는 정식
 * 표기가 낫다 — 화면에 그대로 보이는 값이기 때문이다.
 */
